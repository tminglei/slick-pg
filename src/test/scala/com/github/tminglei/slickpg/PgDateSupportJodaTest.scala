...
class PgDateSupportJodaTest {
  ...
  object DatetimeTable extends Table[DatetimeBean]("DatetimeJodaTest") {
    ...
    def * = id ~ date ~ time ~ datetime ~ datetimetz ~ interval <> (DatetimeBean, DatetimeBean.unapply _)
  }
  ...
  def testDatetimeFunctions(): Unit = {
    db withSession { implicit session: Session =>
      ...
      val q341 = DatetimeTable.filter(_.id === 101L.bind).map(r => r.datetimetz.age(Functions.currentDate.asColumnOf[DateTime]))
      ...
      assertEquals(DateTime.parse("2001-01-03 00:00:00.0000+08", tzDateTimeFormatter), q36.first())
    }
  }
