...
class PgDate2bpSupportTest {
  ...
  def testPlainDateFunctions(): Unit = {
    ...
    db withSession { implicit session: Session =>
      Try { Q.updateNA("drop table if exists DatetimebpTest cascade").execute }
      Try {
        Q.updateNA("create table DatetimebpTest("+
      ...
      (Q.u + "insert into DatetimebpTest values(" +?dateBean.id + ", " +? dateBean.date + ", " +? dateBean.time
        + ", " +? dateBean.dateTime + ", " +? dateBean.dateTimetz + ", " +? dateBean.duration + ", " +? dateBean.period + ")").execute

      val found = (Q[DatetimeBean] + "select * from DatetimebpTest where id = " +? dateBean.id).first
