...
class PgDate2SupportTest {
  ...
  def testDatetimeFunctions(): Unit = {
    db withSession { implicit session: Session =>
      ...
      println(s"[date2] '+' sql = ${q1.selectStatement}")
      ...
      println(s"[date2] '+' sql = ${q101.selectStatement}")
      ...
      println(s"[date2] '+++' sql = ${q2.selectStatement}")
      ...
      println(s"[date2] '+++' sql = ${q3.selectStatement}")
      ...
      println(s"[date2] '--' sql = ${q6.selectStatement}")
      ...
      assertEquals(Duration.parse("PT-86170H-39M0.223571S"), q8.first)
      ...
      println(s"[date2] '-' sql = ${q10.selectStatement}")
      ...
      println(s"[date2] '---' sql = ${q13.selectStatement}")
      ...
      println(s"[date2] 'justifyInterval' sql = ${q28.selectStatement}")
      ...
      println(s"[date2] 'age' sql = ${q34.selectStatement}")
      println(s"[date2] 'age' sql1 = ${q341.selectStatement}")
      assertEquals(q341.first, q34.first)

      val q35 = Datetimes.filter(_.id === 101L.bind).map(r => r.dateTimetz.part("year"))
      println(s"[date2] 'part' sql = ${q35.selectStatement}")
      assertEquals(2001, q35.first, 0.00001d)

      val q36 = Datetimes.filter(_.id === 101L.bind).map(r => r.dateTimetz.trunc("day"))
      println(s"[date2] 'trunc' sql = ${q36.selectStatement}")
      assertEquals(ZonedDateTime.parse("2001-01-03 00:00:00+08", date2TzDateTimeFormatter), q36.first)
    }
  }
  ...
  def testPlainDateFunctions(): Unit = {
    ...
    db withSession { implicit session: Session =>
      Try { Q.updateNA("drop table if exists Datetime2Test cascade").execute }
      Try {
        Q.updateNA("create table Datetime2Test("+
      ...
      (Q.u + "insert into Datetime2Test values(" +?dateBean.id + ", " +? dateBean.date + ", " +? dateBean.time
        + ", " +? dateBean.dateTime + ", " +? dateBean.dateTimetz + ", " +? dateBean.duration + ", " +? dateBean.period + ")").execute

      val found = (Q[DatetimeBean] + "select * from Datetime2Test where id = " +? dateBean.id).first
