...
//      assertEquals(bean, q6.first())
      ...
      val q7 = GeomTestTable.where(_.geom @&& makeBox(point1.bind, point2.bind)).map(t => t)
      assertEquals(bean, q7.first())

      val q8 = GeomTestTable.where(_.geom === makePoint((-71.064544D).bind, (42.28787D).bind)).map(t => t)
  ...
  def testGeomOperators(): Unit = {
    ...
    db withSession { implicit session: Session =>
      ...
      ///
      val q7 = GeomTestTable.where(r => { r.id === id.bind && (r.geom &< line2.bind) }).map(r => r)
      assertEquals(bean, q7.first())

      val q8 = GeomTestTable.where(r => { r.id === id.bind && (line2.bind &> r.geom) }).map(r => r)
      assertEquals(bean, q8.first())

      val q9 = GeomTestTable.where(r => { r.id === id.bind && (r.geom << point.bind) }).map(r => r)
      assertEquals(bean, q9.first())

      val q10 = GeomTestTable.where(r => { r.id === id.bind && (point.bind >> r.geom) }).map(r => r)
      assertEquals(bean, q10.first())

      val q11 = GeomTestTable.where(r => { r.id === id.bind && (r.geom &<| line2.bind) }).map(r => r)
      assertEquals(bean, q11.first())

      val q12 = GeomTestTable.where(r => { r.id === id.bind && (line2.bind |&> r.geom) }).map(r => r)
      assertEquals(bean, q12.first())

      val q13 = GeomTestTable.where(r => { r.id === id.bind && (r.geom <<| point.bind) }).map(r => r)
      assertEquals(bean, q13.first())

      val q14 = GeomTestTable.where(r => { r.id === id.bind && (point.bind |>> r.geom) }).map(r => r)
  ...
  def testGeomAccessors(): Unit = {
    ...
    db withSession { implicit session: Session =>
      ...
      val collectionId = GeomTestTable.iCols.insert(collection)

      val q1 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.geomType)
      assertEquals("ST_LineString", q1.first())

      val q2 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.srid)
      assertEquals(0, q2.first())

      val q3 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.isValid)
      assertEquals(false, q3.first())

      val q4 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.isClosed)
      assertEquals(true, q4.first())

      val q5 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.isCollection)
      assertEquals(false, q5.first())

      val q6 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.isEmpty)
      assertEquals(false, q6.first())

      val q7 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.isRing)
      assertEquals(false, q7.first())

      val q8 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.isSimple)
      assertEquals(true, q8.first())

      val q9 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.hasArc)
      assertEquals(false, q9.first())

      val q10 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.area)
      assertEquals(0.0f, q10.first(), 0.001f)

      val q11 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.boundary)
      assertEquals(wktReader.read("MULTIPOINT(0 0, 3 3)"), q11.first())

      val q12 = GeomTestTable.where(_.id === collectionId.bind).map(_.geom.dimension)
      assertEquals(1, q12.first())

      val q13 = GeomTestTable.where(_.id === collectionId.bind).map(_.geom.coordDim)
      assertEquals(2, q13.first())

      val q14 = GeomTestTable.where(_.id === collectionId.bind).map(_.geom.nDims)
      assertEquals(2, q14.first())

      val q15 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.nPoints)
      assertEquals(2, q15.first())

      val q16 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.nRings)
  ...
  def testGeomOutputs(): Unit = {
    ...
    db withSession { implicit session: Session =>
      val id = GeomTestTable.iCols.insert(polygon)
      val id1 = GeomTestTable.iCols.insert(point)

      val q1 = GeomTestTable.where(_.id === id.bind).map(_.geom.asText)
      assertEquals(POLYGON, q1.first())

      val q2 = GeomTestTable.where(_.id === id.bind).map(_.geom.asEWKT)
      assertEquals(POLYGON_EWKT, q2.first())

      val q3 = GeomTestTable.where(_.id === id.bind).map(_.geom.asHEXEWKB())
      assertEquals(POLYGON_HEX, q3.first())

      val q4 = GeomTestTable.where(_.id === id1.bind).map(_.geom.asLatLonText())
      assertEquals(POINT_LatLon, q4.first())

      val q5 = GeomTestTable.where(_.id === id.bind).map(_.geom.asGML())
      assertEquals(POLYGON_GML, q5.first())

      val q6 = GeomTestTable.where(_.id === id.bind).map(_.geom.setSRID(4326).asKML())
      assertEquals(POLYGON_KML, q6.first())

      val q7 = GeomTestTable.where(_.id === id.bind).map(_.geom.asGeoJSON())
      assertEquals(POLYGON_JSON, q7.first())

      val q8 = GeomTestTable.where(_.id === id.bind).map(_.geom.asSVG())
  ...
  def testGeomRelationships(): Unit = {
    ...
    db withSession { implicit session: Session =>
      val polygonId = GeomTestTable.iCols.insert(polygon)
      val lineId = GeomTestTable.iCols.insert(line1)

      val q1 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.gEquals(line2.bind))
      assertEquals(true, q1.first())
      val q2 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.gEquals(line2.bind.reverse))
      assertEquals(true, q2.first())

      val q3 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.orderingEquals(line2.bind))
      assertEquals(false, q3.first())

      val q4 = GeomTestTable.where(_.id === polygonId.bind).map(
        _.geom.overlaps(geomFromText("LINESTRING(0 0, 10 10)".bind).buffer(10f.bind)))
      assertEquals(false, q4.first())

      val q5 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.intersects(line1.bind))
      assertEquals(true, q5.first())

      val q6 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.crosses(line1.bind))
      assertEquals(true, q6.first())

      val q7 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.disjoint(line1.bind))
      assertEquals(false, q7.first())

      val q8 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.contains(multiPoints.bind))
      assertEquals(true, q8.first())

      val q9 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.containsProperly(multiPoints.bind))
      assertEquals(false, q9.first())

      val q11 = GeomTestTable.where(_.id === polygonId.bind).map(r => multiPoints.bind.within(r.geom))
      ...
      val q15 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.relate(multiPoints.bind, "T*****FF*".bind))
      assertEquals(true, q15.first())
      val q16 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.relatePattern(point.bind))
  ...
  def testGeomMeasures(): Unit = {
    ...
    db withSession { implicit session: Session =>
      val pointId = GeomTestTable.iCols.insert(point1)
      val lineId = GeomTestTable.iCols.insert(line)
      ...
      val q4 = GeomTestTable.where(_.id === pointId.bind).map(_.geom.pointOnSurface)
      assertEquals(point1, q4.first())

      val q5 = GeomTestTable.where(_.id === pointId.bind).map(_.geom.project(1000f.bind, 45f.bind.toRadians))
      assertEquals(projectedPoint, q5.first())

      val q6 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.length)
      assertEquals(122.630744000095d, q6.first(), 0.1d)

      val q7 = GeomTestTable.where(_.id === line3dId.bind).map(_.geom.length3d)
      assertEquals(122.704716741457d, q7.first(), 0.1d)

      val q8 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.perimeter)
      ...
      val q11 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.maxDistance(point2.bind))
      assertEquals(111.80339813232422d, q11.first(), 0.1d)

      val q12 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.hausdorffDistance(line3d.bind))
      assertEquals(0.0d, q12.first(), 0.1d)

      val q13 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.longestLine(point2.bind))
      assertEquals(longestLine, q13.first())

      val q14 = GeomTestTable.where(_.id === polygonId.bind).map(_.geom.shortestLine(point2.bind))
  ...
  def testGeomProcessing(): Unit = {
    ...
    db withSession { implicit session: Session =>
      point.setSRID(4326)
      val pointId = GeomTestTable.iCols.insert(point)
      val lineId = GeomTestTable.iCols.insert(line)
      val multiLineId = GeomTestTable.iCols.insert(multiLine)
      val collectId = GeomTestTable.iCols.insert(collect)

      val q1 = GeomTestTable.where(_.id === pointId.bind).map(_.geom.setSRID(0.bind).asEWKT)
      assertEquals("POINT(-123.365556 48.428611)", q1.first())

      val q2 = GeomTestTable.where(_.id === pointId.bind).map(_.geom.transform(26986.bind).asEWKT)
      assertEquals("SRID=26986;POINT(-3428094.64636768 2715245.01412978)", q2.first())

      val q3 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.simplify(0.5f.bind).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,2 1)", q3.first())

      val q4 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.simplifyPreserveTopology(0.5f.bind).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2,2 1)", q4.first())

      val q5 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.removeRepeatedPoints.asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2,2 1)", q5.first())

      val q6 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.difference(line1.bind).asText)
      assertEquals("MULTILINESTRING((1 1,1.5 1.5),(1.5 1.5,2 2,2 3.5,1 3,1 2))", q6.first())

      val q7 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.symDifference(line1.bind).asText)
      assertEquals("MULTILINESTRING((1 1,1.5 1.5),(1.5 1.5,2 2,2 3.5,1 3,1 2),(2 1,3 1))", q7.first())

      val q8 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.intersection(line1.bind).asText)
      assertEquals("MULTILINESTRING((1 2,1.5 1.5),(1.5 1.5,2 1))", q8.first())

      val q9 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.sharedPaths(line1.bind).asText)
      assertEquals("GEOMETRYCOLLECTION(MULTILINESTRING((1 2,1.5 1.5),(1.5 1.5,2 1)),MULTILINESTRING EMPTY)", q9.first())

      val q10 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.split(bladeLine.bind).asText)
      ...
      val q11 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.minBoundingCircle(8.bind).asText)
      ...
      val q12 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.buffer(10f.bind).asText)
      ...
      val q13 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.multi.asText)
      assertEquals("MULTILINESTRING((1 1,2 2,2 3.5,1 3,1 2,2 1))", q13.first())

      val q14 = GeomTestTable.where(_.id === multiLineId.bind).map(_.geom.lineMerge.asText)
      assertEquals("LINESTRING(-29 -27,-30 -29.7,-36 -31,-45 -33,-46 -32)", q14.first())

      val q15 = GeomTestTable.where(_.id === collectId.bind).map(_.geom.collectionExtract(2.bind).asText)
      assertEquals("MULTILINESTRING((0 0,1 1),(2 2,3 3))", q15.first())

      val q16 = GeomTestTable.where(_.id === collectId.bind).map(_.geom.collectionHomogenize)
      assertEquals("MULTILINESTRING ((0 0, 0 1), (2 2, 3 3))", wktWriter.write(q16.first()))

      val q17 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.addPoint(point1.bind).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2,2 1,3 1)", q17.first())

      val q18 = GeomTestTable.where(_.id === lineId.bind).map(r => r.geom.setPoint(point1.bind, (r.geom.nPoints - 1.bind)).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2,3 1)", q18.first())

      val q19 = GeomTestTable.where(_.id === lineId.bind).map(r => r.geom.removePoint((r.geom.nPoints - 1.bind)).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2)", q19.first())

      val q20 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.scale(1f.bind, 2f.bind).asText)
      assertEquals("LINESTRING(1 2,2 4,2 7,1 6,1 4,2 2)", q20.first())

      val q21 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.segmentize((5f.bind)).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2,2 1)", q21.first())

      val q22 = GeomTestTable.where(_.id === lineId.bind).map(r => r.geom.snap(line1.bind, r.geom.distance(line1.bind)).asText)
      assertEquals("LINESTRING(1 1,2 2,2 3.5,1 3,1 2,2 1)", q22.first())

      val q23 = GeomTestTable.where(_.id === lineId.bind).map(_.geom.scale(1f.bind, 2f.bind).asText)
